            ...
            {!loading &&
              pageItems.map((app) => (
                <tr key={app.id} className="border-t">
                  ...
                  <td className="px-3 py-2 text-xs text-gray-600">{formatDate(app.created_at)}</td>

                  <td className="px-3 py-2">
                    <div className="flex items-center justify-end gap-2">
                      {/* ✅ View uses injected base path */}
                      <Link
                        href={`${detailsBasePath}/${app.id}`}
                      ...
                      { allowConvert && app.status === "accepted" && !app.converted_to_student && (
                        <button
                          ...
                          className={`px-3 py-1 text-xs rounded text-white ${
                            reviewingId === app.id ? "bg-blue-300 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
                          }`}
                        >
        ...
        {!loading && total > 0 && (
          <div className="flex items-center justify-between px-4 py-3 border-t bg-gray-50 text-xs text-gray-600">
            <div>
              ...
              <span className="font-medium">{Math.min(startIndex + PAGE_SIZE, total)}</span> of{" "}
