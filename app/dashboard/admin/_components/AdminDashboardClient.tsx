...
function StatItem({ title, value, icon, href, accent = "neutral" }: StatItemProps) {
        ...
        <div>
          ...
          <p
            className={cx(
              ...
              accent === "red" ? "text-slate-900" : "text-slate-900"
            )}
          >
            {value}
          </p>
  ...
  return href ? (
    <Link
      href={href}
      className="block focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:ring-offset-2 rounded-3xl"
    >
      {card}
    </Link>
  ) : (
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          <StatItem
            ...
            value={formatNGN(data.money.approvedAllTime)}
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200/60 bg-white shadow-xl shadow-slate-100/50">
            ...
            <div className="flex items-start justify-between gap-4 sm:items-center">
              <div>
                ...
                <p className="mt-0.5 text-sm text-slate-600">
                  Active session is highlighted below
                </p>
              </div>
                ...
                {data.bySession.length === 0 ? (
                  ...
                  data.bySession.map((s) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        <span className="inline-flex items-center justify-center rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-700">
          ...
          <div className="border-t border-slate-100 bg-slate-50/50 px-6 py-3">
            <p className="text-xs text-slate-500">
              <span className="font-semibold text-slate-600">Note:</span> Revenue calculated from
              approved_amount with fallback to amount_paid for approved receipts
            </p>
          </div>
        </div>
      </div>
