...
function ProgramImage({ program }: ProgramImageProps) {
  ...
  if (broken || !slug) {
    return (
      <div className="flex h-28 items-center justify-center rounded-2xl bg-gradient-to-br from-primary-600 to-primary-700 text-lg font-semibold text-white">
        {initials}
      </div>
    );
  }
  ...
  return (
    <div className="relative h-28 overflow-hidden rounded-2xl bg-primary-600/10">
      <Image
        src={src}
        alt={program.name}
        fill
...
export default function ProgramsPage() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      if (deptError) {
        ...
        setDepartments((deptData ?? []) as DepartmentSummary[]);
      }
      ...
      if (programError) {
        ...
        const deptMap = new Map(
          (deptData ?? []).map((d: any) => [d.id as string, d as DepartmentSummary]),
        );

        const withDept: Program[] = (programData ?? []).map((p: any) => ({
          ...(p as ProgramRow),
  ...
  const handleProgramCreated = (row: ProgramRow) => {
  ...
  const handleProgramUpdated = (row: ProgramRow) => {
    const dept = row.department_id
        ? departments.find((d) => d.id === row.department_id) ?? null
        : null;
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="h-8 rounded-xl border border-border bg-background px-3 text-xs outline-none transition-colors hover:border-primary/40 focus-visible:border-primary focus-visible:ring-1 focus-visible:ring-primary"
            >
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {prog.department && (
                            <Badge
                              variant="outline"
                              className="border-primary/30 bg-primary/5 text-[10px] font-medium uppercase tracking-wide text-primary"
                            >
